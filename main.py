...
# --- 📰 MODULE 3: NEWS (Alpha Vantage Sentiment) ---
def fetch_news_sentiment(session, av_key):
    """Fetches numeric sentiment score for Gold (GLD)."""
    if not av_key or av_key == "YOUR_KEY": return {}
    try:
        ...
        if "feed" in resp and resp["feed"]:
            # Average sentiment of the latest 5 relevant articles
            scores = [float(item['overall_sentiment_score']) for item in resp['feed'][:5]]
            return {'news_sentiment_score': round(sum(scores) / len(scores), 3)}
    except: pass
    return {}
...
def main():
    ...
    with requests.Session() as s:
        s.headers.update({"User-Agent": "GoldPro/5.0"})
        ...
        row.update(fetch_news_sentiment(s, os.getenv("ALPHA_VANTAGE_API_KEY")))
    ...
    logging.info("🏁 Master Pulse Complete (AV Sentiment Integrated).")
