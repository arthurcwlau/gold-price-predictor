...
def fetch_fred_data(session, api_key):
    ...
    for key, series_id in fred_series.items():
        try:
            ...
            logging.error(f"FRED error for {key}: {e}")
    return fred_data
...
def fetch_polymarket_data(session):
    ...
    for prefix, slug in slugs.items():
        try:
            ...
            logging.error(f"Polymarket error for {slug}: {e}")
    return results

# --- 🏗️ PERSISTENCE & COLUMN GROUPING ---
def main():
    ...
    # 1. FIX: Mapping for Recession (Moves data to the clean names)
    mapping = {
    ...
    # 2. GROUPING LOGIC
    yfinance_list = ['gold_price', 'oil_wti', 'dxy_index', 'vix_index', 'gold_vix', 'real_yield_proxy', 'silver_price', 'gold_miners', 'copper_price', 'gld_etf_vol', 'dxy_vol', 'treasury_10y']
    fred_list = ['inflation_expectation', 'yield_curve_spread', 'real_yield_10y', 'fed_balance_sheet', 'credit_stress_spread']
    metadata = ['date']
    
    # Polymarket columns are everything else except junk names
    junk = [c for c in df_final.columns if 'recession_us_recession' in c]
    poly_list = sorted([c for c in df_final.columns if c not in metadata + yfinance_list + fred_list + junk])
    
    # Reorder the DataFrame
    ordered_cols = [c for c in metadata + yfinance_list + fred_list + poly_list if c in df_final.columns]
    df_final = df_final[ordered_cols]
    
    df_final['date'] = df_final['date'].dt.strftime("%Y-%m-%d %H:%M Z")
    df_final.to_csv(file_name, index=False)
    logging.info(f"🏁 Update Complete. Grouping applied. File saved.")
