...
def get_institutional_data():
    ...
    def fetch_event_markets(slug):
        try:
            # Gamma API: Metadata & Probabilities
    ...
    def get_clob_metrics(token_id):
        ...
        try:
            ...
            # 1. Spread (Lower = More Liquid/Professional)
            spread = 0.0
            if bids and asks:
                spread = round(float(asks[0]['price']) - float(bids[0]['price']), 4)
            
            # 2. Depth (Sum of top 5 levels of liquidity - "The Moat")
    ...
    def process_institutional_curve(slug, prefix, entry_dict):
        ...
        for m in markets:
            ...
            # A. Probability (Gamma API)
            prices = m.get('outcomePrices')
            if isinstance(prices, str): prices = json.loads(prices)
            entry_dict[f"{prefix}_{clean_name}_prob"] = round(float(prices[0]) * 100, 2)
            
            # B. Volume & Open Interest (Gamma/Analytics API)
            # Open Interest = Total value of current active bets
            entry_dict[f"{prefix}_{clean_name}_vol"] = round(float(m.get('volume', 0)), 2)
            entry_dict[f"{prefix}_{clean_name}_oi"] = round(float(m.get('openInterest', 0)), 2)
            
            # C. Spread & Depth (CLOB API)
            # We fetch metrics for the 'Yes' token (usually index 0)
...
if os.path.exists(file_name):
    ...
    # Ensure all new columns (Spread, Depth, OI) are included
...
print(f"🏁 Strategic Data Saved. Captured {len(df_new.columns)} deep-market metrics.")
