...
def generate_backtest_report(file_name="gold_investment_pro.csv"):
    ...
    # FIX: De-fragment the dataframe memory to remove the PerformanceWarning
    df = df.copy()

    # 2. Dynamic Tier Mapping
    tier_midpoints = {
    ...
    # FIX: Changed '1H' (uppercase) to 'h' (lowercase) for Pandas 2.0+ compatibility
    df_h = df[['gold_price', 'pred_now']].copy()
    df_h = df_h.resample('h').mean().ffill()
    ...
    for lag in lags:
        if f'forecast_{lag}h' in df_h.columns:
    ...
    for lag in lags:
        if f'error_{lag}h' in df_h.columns:
